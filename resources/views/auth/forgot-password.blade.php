<x-guest-layout>
    <div class="card w-96 bg-base-100 shadow mx-auto mt-6">
        <div class="card-body">
            <h2 class="text-xl font-bold mb-2 text-white">@lang('Forgot Password')</h2>
            <p class="text-sm mb-3 text-white">{{ __('Forgot your password? Enter your email and we will send a reset link.') }}</p>
            @if (session('status'))
            ...
            <form method="POST" action="{{ route('password.email') }}" class="space-y-3">
                ...
                <div class="form-control">
                    <label class="label" for="email"><span class="label-text text-white">@lang('Email')</span></label>
